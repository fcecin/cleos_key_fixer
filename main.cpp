...
string key_search_inner(const string & wallet_key_str, int level, bool toplevel = true) {
  private_key_type pkey;
  for (unsigned i = 0; i < 51; ++i) {
    string kstr = wallet_key_str;
    for (unsigned c = 0; c < sizeof(base58_chars); ++c) {
      kstr[i] = base58_chars[c];
      try {
        pkey = private_key_type( kstr );
        return kstr;
      } catch (...) {
        if (level > 1) {
          string rkstr = key_search_inner( kstr, level-1, false );
          if (rkstr.length() > 0)
            return rkstr;
        }
      }
...
string key_search_delete(const string & wallet_key_str, int level, bool toplevel = true) {
  private_key_type pkey;
  for (unsigned i = 0; i < wallet_key_str.length(); ++i) {
    string kstr = wallet_key_str;
    kstr.erase(kstr.begin() + i);
    if (kstr.length() > 51) {
      string dkstr = key_search_delete( kstr, level, false );
      if (dkstr.length() > 0)
        return dkstr;
    } else {
      try {
        ...
        if (level > 0) {
          string ikstr = key_search_inner( kstr, level, false );
          if (ikstr.length() > 0)
            return ikstr;
        }
      }
...
string key_search_insert(const string & wallet_key_str, int level, bool toplevel = true) {
  private_key_type pkey;
  for (unsigned i = 0; i < wallet_key_str.length(); ++i) {
    string kstr = wallet_key_str;
    kstr.insert(kstr.begin() + i, ' ');
    for (unsigned c = 0; c < sizeof(base58_chars); ++c) {
      kstr[i] = base58_chars[c];
      if (kstr.length() < 51) {
        string ikstr = key_search_insert( kstr, level, false );
        if (ikstr.length() > 0)
          return ikstr;
      } else {
        try {
...
//                                ->check(CLI::ExistingFile);
   ...
   importWallet->add_flag("--fix-key", fix_key, localized("Attempt to fix an invalid private key"));
   importWallet->callback([&wallet_name, &wallet_key_str, &fix_key] {
      ...
      try {
        ...
        std::cout << localized("Given private key is invalid. Attempting to fix it.") << std::endl;

        int level = 0;
        while (true) {

          std::cout << "**** " << localized("Typo search depth: ") << level << " ****" << std::endl;
          
          string fixed_key_str;
          int len = wallet_key_str.length();

          if (len == 51) {
            if (level > 0)
              fixed_key_str = key_search_inner(wallet_key_str, level);
          } else if (len > 51) {
            fixed_key_str = key_search_delete(wallet_key_str, level);
          } else if (len < 51) {
            fixed_key_str = key_search_insert(wallet_key_str, level);
          }

          // If level is low enough, then also do a top-level search
          //  flipping two adjacent characters in the original key string.
          if (fixed_key_str.length() == 0 && level <= 1) {
            for (int i=0; i<len-1; ++i) {
              string fkstr = wallet_key_str;

              // flip adjacent characters at i and i+1
              char ch = fkstr[i];
              fkstr[i] = fkstr[i+1];
              fkstr[i+1] = ch;
              ...
              if (len == 51) {
                try {
                  wallet_key = private_key_type( fkstr );
                  fixed_key_str = fkstr;
                } catch (...) {
                  if (level > 0)
                    fixed_key_str = key_search_inner(fkstr, level, false);
                }
              } else if (len > 51) {
                fixed_key_str = key_search_delete(fkstr, level, false);
              } else if (len < 51) {
                fixed_key_str = key_search_insert(fkstr, level, false);
              }
              
              if (fixed_key_str.length() > 0)
                break;
          ...
          if (fixed_key_str.length() > 0) {
            try {
              wallet_key = private_key_type( fixed_key_str );
            } catch (...) {
              EOS_THROW(private_key_type_exception, "Private key fixer is broken.");
            }
          
            public_key_type pubkey = wallet_key.get_public_key();
            std::cout << std::endl << localized("Fixed private key!") << std::endl;
            std::cout << localized("Public key: ${pubkey}", ("pubkey", pubkey.to_string())) << std::endl;
            std::cout << localized("Private key (fixed): ${wallet_key}", ("wallet_key", wallet_key.to_string())) << std::endl;
            std::cout << localized("Private key (wrong): ${wallet_key_str}", ("wallet_key_str", wallet_key_str)) << std::endl;
